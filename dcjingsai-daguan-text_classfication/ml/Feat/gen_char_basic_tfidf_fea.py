...
def main():
    ...
    if 0 in step_list:
        ...
        for i, (train_index, test_index) in enumerate(skf, 1):
            ...
            vec = TfidfVectorizer(max_features=2000000, ngram_range=(1,4),min_df=5, max_df=0.5,use_idf=1,smooth_idf=1, sublinear_tf=1)
            train_off_term_doc = vec.fit_transform(train_off[feature_name])
            print(train_off_term_doc.shape)

            train_off_feat_file = '{0}/train_off.{1}.feat.pkl'.format(path, feat_name)
            ...
            test_off_feat_file = '{0}/test_off.{1}.feat.pkl'.format(path, feat_name)
        ...
        train_feat_file =  '{0}/train.{1}.feat.pkl'.format(config.feat_folder, feat_name)
        if not os.path.isfile(train_feat_file):
            with open(train_feat_file, 'wb') as f:
                cPickle.dump(train_term_doc, f)

        test_feat_file = '{0}/test.{1}.feat.pkl'.format(config.feat_folder, feat_name)
